    ...
    async def run(self, context, agent=None):
        ...
        system = SystemMessage(
            content=(
                ...
                "Judge completion by whether the step's GOAL was achieved (e.g. emails were retrieved), "
                "not by whether a specific tool name was called. If a tool_result satisfies the step's "
                "intent, choose advance — do not try to call a different tool just because the step "
                "description mentioned a different name.\n\n"
                "A step often needs MULTIPLE tool calls when one call provides info needed for the next "
                "(e.g. list calendars to get an ID, then create the event). But a step that asks to "
                "'retrieve emails' is done once emails are in the tool_result — advance immediately.\n\n"
